...
void print_graphical_error()
{
    if(pthread_mutex_unlock(&g_mutex) == ERR)
        exit_program();

    list_add_last_element(&g_list, strerror(errno));
...
void* command_handler()
{
    ...
    while(1)
    {
        ...
        if(len > 0)
        {
            ...
            list_add_last_element(&g_list, t_readed_command);
